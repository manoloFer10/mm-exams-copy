...
from model_utils import parse_qwen_inputs, parse_openai_input, format_answer, temperature, max_tokens, SYSTEM_MESSAGE, SUPPORTED_MODELS
import torch
...
def predict_gpt4 (client, json_schema: Dict, system_message: List[Dict[str,str]], model: str = 'gpt-4o' , max_tokens: int = 128, temperature: float = 0):
    '''
    ZERO-SHOT
    ...
    question, parsed_options = parse_openai_input(question, question_image, options_list)

    question_text_message = {
        'role': 'user',
        'content': question + parsed_options
    }
...
def run_answer_prediction(model: str, data_path: str, API_KEY: str):
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(f'Invalid path for running model prediction: {data_path}')
    ...
    if model not in SUPPORTED_MODELS:
        raise NotImplementedError(f'Model {model} not implemented for prediction in this code.')

    if model == 'gpt-4o':
        client = OpenAI(api_key=API_KEY)
        for question_json in test_questions:
            prediction = predict_gpt4(client, question_json, system_message, temperature=temperature, max_tokens = max_tokens)
    ...
    if model == 'qwen':
        ...
        for question_json in test_questions:
            prediction = predict_qwen(qwen, qwen_processor, question_json, system_message)
            question_json['prediction_by_'+model] = prediction
