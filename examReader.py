...
class examProcessor:
    ...
    def evaluate_model(self, model: Replier):
        ...
        for filename, loader in self.exam_loaders:
            ...
            accuracy = self._evaluate_single_exam(model, questions)
            results[filename] = accuracy
        return results

    def _evaluate_single_exam(self, model: Replier, questions: list[dict]):
        """Evaluates a model on a single exam."""
        correct = 0
        total = len(questions)
        for question in questions:
            predicted_answer = model.predict_zero_shot(question["question"], question["options"])
